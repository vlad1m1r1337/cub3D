...
/*   Updated: 2023/09/28 16:25:35 by vgribkov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void raycasting(t_mlx *mlx)
{
	for (int x = 0; x < W; x++)
	{
		...
		double wallX;
		if(mlx->side == 0) wallX = mlx->pos_y + mlx->perp_wall_dist * mlx->ray_dir_y;
		else          wallX = mlx->pos_x + mlx->perp_wall_dist * mlx->ray_dir_x;
		wallX -= floor((wallX));

		int texX = (int)(wallX * (double)(texWidth));
		if(mlx->side == 0 && mlx->ray_dir_x > 0) texX = texWidth - texX - 1;
		if(mlx->side == 1 && mlx->ray_dir_y < 0) texX = texWidth - texX - 1;
